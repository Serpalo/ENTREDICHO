...
const ProjectDetail = ({ projects = [] }: any) => {
  ...
  // Buscamos el folleto comparando IDs como texto para evitar errores de tipo (uuid vs int)
  ...
  if (!project) return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center">
      <div className="text-center font-black uppercase text-slate-300 italic tracking-widest animate-pulse">
        Cargando datos del folleto...
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-slate-50 p-10 font-sans">
      <div className="flex justify-between items-center mb-10">
        <div className="flex gap-6 items-center">
          <button onClick={() => navigate(-1)} className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 hover:bg-slate-50 transition-all active:scale-90">
        ...
        <div className="flex gap-2 bg-white p-2 rounded-2xl shadow-sm border border-slate-100">
          {[1, 2, 3].map((v) => (
            <button key={v} onClick={() => setCurrentVersion(v)}
              className={`px-6 py-2 rounded-xl font-black text-[10px] uppercase transition-all ${currentVersion === v ? 'bg-[#1e293b] text-white shadow-lg' : 'text-slate-400 hover:bg-slate-50'}`}>
            ...
            {project.image_url ? (
              ...
              <div className="aspect-[3/4] bg-slate-50 rounded-[2rem] flex items-center justify-center border-2 border-dashed border-slate-200">
                <span className="text-slate-300 font-black italic uppercase tracking-widest text-center px-10">
                  Sin imagen previsualizable para este folleto
                </span>
              </div>
            )}
          </div>
        ...
        <div className="col-span-5 space-y-8">
          <div className="bg-white p-10 rounded-[3rem] shadow-sm border border-slate-100">
            ...
            <textarea 
              ...
              className="w-full p-8 bg-slate-50 rounded-[2rem] border-none text-sm font-medium mb-6 min-h-[150px] focus:ring-2 focus:ring-rose-500"
            ...
            <div className="flex gap-4">
              ...
              <label htmlFor="adjunto" className="flex-1 text-center py-4 bg-slate-100 text-slate-500 rounded-2xl font-black text-[10px] uppercase cursor-pointer hover:bg-slate-200 transition-colors">
                {selectedFile ? "✓ ADJUNTO LISTO" : "📎 ADJUNTAR IMAGEN"}
              </label>
            ...
            {corrections.map((c) => (
              <div key={c.id} 
                ...
                <input 
                  ...
                  className="mt-1 w-7 h-7 rounded-xl border-slate-300 text-emerald-600 focus:ring-emerald-500 cursor-pointer shadow-sm" 
                />
