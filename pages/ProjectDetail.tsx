...
const ProjectDetail = ({ projects = [] }: any) => {
  ...
  // Cargar correcciones usando tus columnas reales: page_id y content
  const loadCorrections = async () => {
    const { data } = await supabase
      .from('comments')
      .select('*')
      .eq('page_id', projectId); // Usamos page_id como referencia al folleto
    setCorrections(data || []);
  };

  useEffect(() => { if (projectId) loadCorrections(); }, [projectId]);
  ...
  const handleAddNote = async () => {
    ...
    // Insertar usando tus columnas: content, resolved, attachment_url
    await supabase.from('comments').insert([{
      page_id: projectId,
      content: newNote,
      attachment_url: fileUrl,
      resolved: false // Rojo por defecto
    }]);
  ...
  const toggleCheck = async (id: string, currentResolved: boolean) => {
    // Al pulsar el check, cambia el fondo de Rojo a Verde
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-10 font-sans">
      ...
      <div className="grid grid-cols-12 gap-10">
        <div className="col-span-7 bg-white p-6 rounded-[3rem] shadow-sm border border-slate-100 h-fit">
          <img src={project.image_url} alt="Folleto" className="w-full h-auto rounded-[2rem]" />
        </div>
        ...
        <div className="col-span-5 space-y-6">
          <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={handleAddNote} className="flex-1 py-4 bg-rose-600 text-white rounded-xl font-black text-[9px] uppercase shadow-lg">
                GUARDAR
              </button>
            </div>
          </div>
