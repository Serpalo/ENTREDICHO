...
const ProjectDetail: React.FC<ProjectDetailProps> = ({ projects, setProjects }) => {
  ...
  const fetchCommentsCount = async () => {
    ...
    // --- CAMBIO CLAVE: Contar solo las que NO están resueltas (resolved = false) ---
    const { data } = await supabase
      .from('comments')
      .select('page_id')
      .in('page_id', pageIds)
      .eq('resolved', false); // <--- SOLO CUENTA LAS PENDIENTES
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="max-w-7xl mx-auto px-8 py-10">
        {currentVersion && currentVersion.pages.length > 0 ? (
            ...
            {viewMode === 'grid' ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                {currentVersion.pages.map((page, index) => {
                  ...
                  return (
                    <div key={page.id} onClick={() => navigate(`/project/${project.id}/version/${currentVersion.id}/page/${page.id}`)} className="group bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-xl hover:border-indigo-200 transition-all cursor-pointer overflow-hidden relative aspect-[3/4]">
          ...
          <div className="py-20 text-center border-2 border-dashed border-slate-200 rounded-3xl"><p className="text-slate-400 font-medium">Este proyecto no tiene páginas visibles.</p></div>
        )}
      </main>
    </div>
  );
