...
const PageReview: React.FC<PageReviewProps> = ({ projects, setProjects, addNotification }) => {
  ...
  const handleWheel = (e: React.WheelEvent) => {
    // Si estamos escribiendo una nota, no hacemos zoom
    if (tempPin) return;
    
    e.preventDefault();
    const scaleAmount = -e.deltaY * 0.001;
    const newScale = Math.min(Math.max(0.5, transform.scale + scaleAmount), 5); // Min 0.5x, Max 5x
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    // Si estamos en modo chincheta, el clic es para poner el punto, no para arrastrar
    if (isPinMode) return;
  ...
  // --- LÓGICA DE CHINCHETAS ---

  const handleCanvasClick = (e: React.MouseEvent<HTMLDivElement>) => {
    if (!isPinMode || !imageContainerRef.current) return;
    
    // Calcular coordenadas relativas a la imagen (compensando el zoom)
      ...
      <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-6 flex-shrink-0 z-50">
        ...
        <div className="flex items-center gap-3">
            ...
            <div className="flex bg-slate-100 rounded-lg p-1 mr-2">
                ...
                <button onClick={resetView} className="ml-1 p-2 hover:bg-white rounded shadow-sm text-indigo-500 font-bold text-xs" title="Resetear">1:1</button>
            </div>
           ...
           >
             ...
             {isPinMode ? 'Cancela Chincheta' : 'Poner Chincheta'}
           </button>
        </div>
      </header>

      <div className="flex-1 flex overflow-hidden relative">
        {/* ÁREA DE VISUALIZACIÓN CON ZOOM */}
        <div 
        ...
        >
          ...
          <div 
            ...
            style={{ 
                ...
                minWidth: '600px', // Tamaño base
                width: 'auto'
            }}
          >
            {isPdf ? (
              // Nota: Los iframes a veces capturan el ratón y rompen el drag. Ponemos pointer-events-none mientras arrastramos.
              <div className={isDragging ? 'pointer-events-none' : ''}>
                  <iframe src={`${page.imageUrl}#toolbar=0`} className="w-[800px] h-[1100px] border-none bg-white block" title="Visor" />
              </div>
            ) : (
              <img src={page.imageUrl} alt="Documento" className="max-w-none block" style={{ height: 'auto', width: 'auto', maxHeight: '90vh' }} />
            )}

            {isPinMode && (
              <div className="absolute inset-0 bg-indigo-500/10 z-10 border-2 border-rose-500 animate-pulse">
                 <div className="absolute top-4 left-1/2 -translate-x-1/2 bg-rose-600 text-white px-4 py-2 rounded-full text-xs font-bold shadow-lg shadow-black/20 transform -translate-y-full">
                   Haz clic para marcar el error
                 </div>
              </div>
            )}
            ...
            {commentsList.map((c) => (
              ...
              >
                 {/* La chincheta se mantiene del mismo tamaño visual invirtiendo la escala? No, mejor que escale para ver dónde está exacto */}
                ...
                {/* Tooltip con el número de nota si quieres */}
                <div className={`absolute -top-6 left-1/2 -translate-x-1/2 bg-white px-2 py-0.5 rounded text-[8px] font-bold shadow whitespace-nowrap opacity-0 group-hover:opacity-100 pointer-events-none ${c.resolved ? 'text-emerald-700' : 'text-rose-700'}`}>
                    {c.content.substring(0, 10)}...
                </div>
              </div>
            ))}
                ...
                {commentsList.map((c) => (
                  ...
                  >
                    <div className="flex items-start gap-3">
                      <div className="mt-1">
                        <input type="checkbox" checked={c.resolved || false} onChange={(e) => { e.stopPropagation(); handleToggleResolve(c.id, c.resolved || false); }} className="w-5 h-5 rounded border-2 border-slate-300 text-emerald-600 focus:ring-emerald-500 cursor-pointer"/>
                      </div>
