  ...
  return (
    <div className="min-h-screen bg-slate-800 flex flex-col h-screen overflow-hidden">
      <header className="h-16 bg-white flex items-center px-6">
          <button onClick={() => navigate(-1)} className="mr-4">Volver</button>
          <h1 className="font-bold">{project.name} - Pág {page.pageNumber}</h1>
          <button onClick={() => setIsPinMode(!isPinMode)} className="ml-auto bg-rose-600 text-white px-4 py-2 rounded">
      ...
      <div className="flex-1 flex overflow-hidden relative">
        <div className="flex-1 overflow-hidden flex items-center justify-center bg-slate-900">
          <div ref={imageContainerRef} onClick={handleCanvasClick} className="relative shadow-2xl" style={{ transform: `scale(${transform.scale})` }}>
        ...
        {tempPin && (
            <div className="w-80 bg-white p-4 border-l">
                <h3>Nueva Nota</h3>
                <textarea className="w-full border p-2 mt-2" value={comment} onChange={e => setComment(e.target.value)} />
                <button onClick={handleSavePin} className="bg-rose-600 text-white w-full py-2 mt-2 rounded">Guardar</button>
            </div>
        )}
      </div>
    </div>
