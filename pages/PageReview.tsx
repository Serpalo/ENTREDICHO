...
const PageReview: React.FC<PageReviewProps> = ({ projects, setProjects, addNotification }) => {
  ...
  // Estados de Zoom, Movimiento y V2
  const [transform, setTransform] = useState({ scale: 1, x: 0, y: 0 });
  const [isDragging, setIsDragging] = useState(false);
  const [startPan, setStartPan] = useState({ x: 0, y: 0 });
  const [isUploadingV2, setIsUploadingV2] = useState(false);
  const [showComparison, setShowComparison] = useState(false); // false = ver V2 (actual), true = ver V1 (anterior)

  const project = projects.find(p => p.id === projectId);
  const version = project?.versions.find(v => v.id === versionId);
  // Extendemos el tipo localmente para evitar errores de TypeScript si types.ts no está actualizado
  const page = version?.pages.find(p => p.id === pageId) as any; 
  ...
  // --- LÓGICA SUBIR V2 ---
  const handleV2Upload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file || !page) return;

    setIsUploadingV2(true);
    const fileName = `v2-${Date.now()}-${file.name.replace(/[^a-zA-Z0-9._-]/g, '_')}`;

    // 1. Subir nueva imagen
    const { error: uploadError } = await supabase.storage.from('brochures').upload(fileName, file);
    if (uploadError) { console.error(uploadError); setIsUploadingV2(false); return; }
    
    const { data: { publicUrl: newUrl } } = supabase.storage.from('brochures').getPublicUrl(fileName);

    // 2. Actualizar DB: Mover actual a 'previous', nueva a 'image_url'
    // IMPORTANTE: Guardamos la URL actual como "previous_image_url" antes de sobrescribirla
    const { error: dbError } = await supabase
      .from('pages')
      .update({ 
        image_url: newUrl,
        previous_image_url: page.imageUrl // La que era actual ahora es la vieja
      })
      .eq('id', page.id);

    if (dbError) { alert('Error actualizando base de datos'); setIsUploadingV2(false); return; }
        ...
        const updatedVersions = p.versions.map(v => {
            ...
            return {
                ...
                pages: v.pages.map(pg => pg.id === pageId ? { 
                    ...pg, 
                    imageUrl: newUrl, 
                    previousImageUrl: page.imageUrl // Actualizamos localmente también
                } : pg)
            };
        });
    ...
    alert('¡V2 Subida con éxito! Ahora puedes comparar.');
  };

  if (!project || !version || !page) return <div className="p-10 text-center">Cargando...</div>;
  
  // Decidir qué imagen mostrar: Si estamos comparando (showComparison es true), mostramos la Vieja. Si no, la Nueva.
  const displayUrl = showComparison && page.previousImageUrl ? page.previousImageUrl : page.imageUrl;
  ...
  return (
    <div className="min-h-screen bg-slate-800 flex flex-col h-screen overflow-hidden">
      <input type="file" ref={fileInputRef} className="hidden" onChange={handleV2Upload} accept="image/*,.pdf" />
      ...
      <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-6 flex-shrink-0 z-50">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
                 ...
                 {page.previousImageUrl && <span className="bg-indigo-100 text-indigo-700 text-[9px] font-bold px-1.5 rounded">CONTIENE V2</span>}
            </div>
          </div>
        </div>
        <div className="flex items-center gap-3">
            ...
            <button 
                ...
                disabled={isUploadingV2}
                className="flex items-center gap-2 px-3 py-2 bg-slate-100 hover:bg-indigo-50 text-slate-600 hover:text-indigo-600 rounded-xl font-bold text-xs transition-all"
                title="Subir corrección (V2)"
            >
                {isUploadingV2 ? <div className="w-4 h-4 border-2 border-indigo-500 border-t-transparent rounded-full animate-spin"></div> : <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" /></svg>}
                {isUploadingV2 ? 'Subiendo...' : 'Subir V2'}
            </button>

            {/* BOTÓN COMPARAR (SOLO SI HAY V2) */}
            {page.previousImageUrl && (
                <button 
                    onMouseDown={() => setShowComparison(true)}
                    onMouseUp={() => setShowComparison(false)}
                    onMouseLeave={() => setShowComparison(false)}
                    className={`flex items-center gap-2 px-3 py-2 rounded-xl font-bold text-xs transition-all select-none ${showComparison ? 'bg-amber-500 text-white shadow-lg scale-105' : 'bg-amber-100 text-amber-700 hover:bg-amber-200'}`}
                >
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                    {showComparison ? 'Viendo V1' : 'Comparar'}
                </button>
            )}
      ...
      <div className="flex-1 flex overflow-hidden relative">
        <div 
            className={`flex-1 overflow-hidden flex items-center justify-center relative bg-slate-900 ${isDragging ? 'cursor-grabbing' : isPinMode ? 'cursor-crosshair' : 'cursor-grab'}`}
            onWheel={handleWheel}
        ...
        >
          ...
          {showComparison && (
              <div className="absolute top-4 left-1/2 -translate-x-1/2 z-50 bg-amber-500 text-white px-6 py-2 rounded-full font-black text-sm shadow-xl animate-pulse pointer-events-none">
                  MOSTRANDO VERSIÓN ANTERIOR (V1)
              </div>
          )}
          ...
          >
            {isPdf ? (
              <div className={isDragging ? 'pointer-events-none' : ''}>
                  <iframe src={`${displayUrl}#toolbar=0`} className="w-[800px] h-[1100px] border-none bg-white block" title="Visor" />
              </div>
            ) : (
              <img src={displayUrl} alt="Documento" className="max-w-none block" style={{ height: 'auto', width: 'auto', maxHeight: '90vh' }} />
            )}

            {isPinMode && (
              <div className="absolute inset-0 bg-indigo-500/10 z-10 border-2 border-rose-500 animate-pulse">
                 <div className="absolute top-4 left-1/2 -translate-x-1/2 bg-rose-600 text-white px-4 py-2 rounded-full text-xs font-bold shadow-lg transform -translate-y-full">Haz clic para marcar el error</div>
              </div>
            )}

            {/* OCULTAMOS LAS NOTAS SI ESTAMOS COMPARANDO PARA VER LIMPIA LA IMAGEN ANTIGUA */}
            {!showComparison && commentsList.map((c) => (
              <div 
                key={c.id}
              ...
              >
                ...
                <div className={`absolute -top-6 left-1/2 -translate-x-1/2 bg-white px-2 py-0.5 rounded text-[8px] font-bold shadow whitespace-nowrap opacity-0 group-hover:opacity-100 pointer-events-none ${c.resolved ? 'text-emerald-700' : 'text-rose-700'}`}>{c.content.substring(0, 10)}...</div>
              </div>
            ))}
