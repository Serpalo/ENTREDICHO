...
const Dashboard = ({ projects = [], folders = [], onRefresh }: any) => {
  ...
  // 1. RED DE SEGURIDAD: Si los datos no han llegado, usamos listas vacías
  const safeFolders = Array.isArray(folders) ? folders : [];
  const safeProjects = Array.isArray(projects) ? projects : [];

  // 2. TÍTULO SEGURO: Evitamos que .find() rompa la web si no hay datos
  ...
  return (
    <div className="p-10 bg-slate-50 min-h-screen font-sans">
      <div className="flex justify-between items-center mb-10 bg-white p-8 rounded-[2rem] shadow-xl border-b-4 border-rose-600">
        ...
        <button className="px-8 py-3 bg-rose-600 text-white rounded-2xl font-black text-[10px] uppercase shadow-lg">
          NUEVO
        </button>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-5 gap-8">
        {/* CARPETAS: Filtro ultra-seguro */}
        {safeFolders
        ...
        {/* PROYECTOS: Sin imágenes complejas para evitar el error de la consola */}
        {safeProjects
