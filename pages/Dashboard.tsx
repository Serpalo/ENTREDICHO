...
const Dashboard = ({ projects, folders, setFolders }: any) => {
        ...
        {currentProjects.map((p: any) => {
          ...
          return (
            <div 
              ...
              className="group bg-white p-5 rounded-[2.5rem] border-2 border-slate-100 hover:shadow-2xl cursor-pointer transition-all hover:-translate-y-2 hover:border-rose-100 flex flex-col h-full"
            >
              <div className="aspect-[3/4] bg-slate-50 rounded-[2rem] mb-6 overflow-hidden border border-slate-50 shadow-inner relative">
                {latestVersion?.pages[0]?.imageUrl ? (
                  <img src={latestVersion.pages[0].imageUrl} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" alt="" />
                ) : (
