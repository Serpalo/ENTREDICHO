  ...
  return (
    <div className="p-10 bg-gray-50 min-h-screen font-sans">
      <div className="flex justify-between items-center mb-10 bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100">
        <h1 className="text-3xl font-black uppercase text-slate-800 italic">{pageTitle}</h1>
        <div className="flex gap-4">
          <button onClick={() => setShowNewFolder(true)} className="px-6 py-3 bg-white border-2 rounded-2xl font-black text-[10px] uppercase">+ Carpeta</button>
          <button onClick={() => fileInputRef.current?.click()} className="px-8 py-3 bg-rose-600 text-white rounded-2xl font-black text-[10px] uppercase">Subir</button>
        </div>
      </div>
      <div className="grid grid-cols-2 md:grid-cols-5 gap-8">
        {folders.filter((f: any) => folderId ? String(f.parentId) === String(folderId) : !f.parentId).map((f: any) => (
          <div key={f.id} className="relative group">
            <button onClick={(e) => handleDelete(e, 'folders', f.id)} className="absolute -top-2 -right-2 bg-rose-600 text-white w-8 h-8 rounded-full z-30 font-bold">✕</button>
            <div onClick={() => navigate(`/folder/${f.id}`)} className="bg-white p-10 rounded-[2.5rem] border-2 cursor-pointer flex flex-col items-center">
        ...
        {projects.filter((p: any) => folderId ? String(p.parentId) === String(folderId) : !p.parentId).map((p: any) => (
          <div key={p.id} className="relative group">
            <button onClick={(e) => handleDelete(e, 'projects', p.id)} className="absolute -top-2 -right-2 bg-rose-600 text-white w-8 h-8 rounded-full z-30 font-bold">✕</button>
            <div onClick={() => navigate(`/project/${p.id}`)} className="bg-white p-5 rounded-[2.5rem] border-2 cursor-pointer flex flex-col h-full">
               <div className="aspect-[3/4] rounded-[2rem] overflow-hidden mb-4 bg-gray-50 border relative">
                  {p.versions?.[p.versions.length - 1]?.pages?.[0]?.imageUrl && <img src={p.versions[p.versions.length - 1].pages[0].imageUrl} className="w-full h-full object-cover" />}
               </div>
