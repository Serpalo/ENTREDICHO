...
const Dashboard = ({ projects = [], folders = [], onRefresh }: any) => {
  ...
  // PROTECCIÓN: Si folders no ha cargado, ponemos un título por defecto
    ...
    <div className="p-10 bg-slate-50 min-h-screen font-sans">
      <div className="flex justify-between items-center mb-10 bg-white p-8 rounded-[2rem] shadow-xl border-b-4 border-rose-600">
        ...
        <div className="flex gap-4">
          <button onClick={() => setShowNewFolder(true)} className="px-6 py-3 bg-slate-100 rounded-2xl font-black text-[10px] uppercase tracking-widest">+ CARPETA</button>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-8">
        {/* CARPETAS - Con protección de filtro */}
        ...
        {/* PROYECTOS - Con protección de filtro */}
        {projects?.filter((p: any) => folderId ? String(p.parentId) === String(folderId) : !p.parentId).map((p: any) => (
          <div key={p.id} className="relative group">
            <div onClick={() => navigate(`/project/${p.id}`)} className="bg-white p-5 rounded-[2.5rem] border-2 cursor-pointer flex flex-col h-full hover:shadow-2xl transition-all">
              <div className="aspect-[3/4] rounded-[1.8rem] overflow-hidden mb-4 bg-slate-100 border">
