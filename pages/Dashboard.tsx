...
const Dashboard = ({ projects = [], folders = [], onRefresh }: any) => {
  ...
  const handleSearch = async (term: string) => {
      ...
      setIsSearching(true);
      // Buscamos en la tabla comments y traemos los datos de la página (projects) relacionada
      const { data, error } = await supabase
        .from('comments')
        .select(`
            *,
            projects:page_id ( id, name, image_url, version )
        `)
        .ilike('content', `%${term}%`) // ilike es "case insensitive" (ignora mayúsculas/minúsculas)
  ...
  // --- FUNCIONES DE SUBIDA Y DESCARGA (IGUAL QUE ANTES) ---
    ...
    return (
      <div className="relative w-full h-full flex flex-col items-center justify-center bg-white rounded-[2rem] shadow-sm border border-slate-100 overflow-hidden group">
        ...
        <div className="absolute bottom-0 w-full bg-white/90 backdrop-blur-sm p-4 border-t border-slate-100 flex justify-between items-center">
             ...
             <div className="flex gap-3"><button onClick={(e) => deleteProject(e, p.id)} className="px-4 py-2 bg-slate-100 text-slate-400 rounded-xl text-xs font-bold hover:bg-red-50 hover:text-red-500 transition-colors">ELIMINAR PÁGINA</button><button onClick={() => navigate(`/project/${p.id}`)} className="px-6 py-2 bg-rose-600 text-white rounded-xl text-xs font-black uppercase hover:scale-105 transition-transform shadow-lg shadow-rose-200">ENTRAR A CORREGIR</button></div>
        </div>
    ...
    <div className="flex min-h-screen bg-slate-50 font-sans">
      <div className="w-64 bg-white border-r border-slate-200 p-8 flex flex-col gap-8">
        ...
        <div className="relative">
            <input type="text" placeholder="Buscar corrección..." value={searchTerm} onChange={(e) => handleSearch(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2 text-xs font-bold text-slate-700 focus:outline-none focus:border-rose-500 transition-all pl-9" />
            <span className="absolute left-3 top-2 text-slate-400 text-xs">🔍</span>
        </div>
        <nav className="flex flex-col gap-2">
          <div onClick={() => { setSearchTerm(""); navigate('/'); }} className="flex items-center gap-3 text-slate-800 font-bold text-sm cursor-pointer p-2 hover:bg-slate-50 rounded-xl">🏠 Inicio</div>
        ...
        {searchTerm.length > 0 ? (
            <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 p-8">
                <div className="flex justify-between items-center mb-6">
                    <h2 className="text-2xl font-black italic text-slate-800 uppercase">Resultados para: "{searchTerm}"</h2>
                    <button onClick={() => setSearchTerm("")} className="text-slate-400 hover:text-rose-600 font-bold text-xs uppercase">Cerrar Búsqueda</button>
                </div>
                
                {isSearching ? (
                    <div className="text-center text-slate-400 font-bold animate-pulse">Buscando en todos los folletos...</div>
                ) : searchResults.length > 0 ? (
                    <div className="grid grid-cols-1 gap-4">
                        {searchResults.map((result: any) => (
                            <div key={result.id} onClick={() => navigate(`/project/${result.page_id}`)} className="p-4 border border-slate-100 rounded-2xl hover:bg-slate-50 hover:border-rose-200 cursor-pointer group transition-all flex items-center gap-4">
                                {/* Miniatura de la página */}
                                {result.projects?.image_url && (
                                    <div className="w-12 h-16 bg-slate-200 rounded-lg overflow-hidden shrink-0 border border-slate-200">
                                        <img src={result.projects.image_url} className="w-full h-full object-cover" />
                                    </div>
                                )}
                                <div className="flex-1">
                                    <div className="flex justify-between mb-1">
                                        <span className="text-[10px] font-black text-rose-400 uppercase">
                                            {result.projects?.name || "Página desconocida"} (V{result.projects?.version})
                                        </span>
                                    ...
                                    <div className="mt-1 flex gap-2">
                                        {result.resolved ? <span className="text-[9px] bg-emerald-100 text-emerald-600 px-2 py-0.5 rounded font-bold">HECHO</span> : <span className="text-[9px] bg-rose-100 text-rose-600 px-2 py-0.5 rounded font-bold">PENDIENTE</span>}
                                        {result.is_general && <span className="text-[9px] bg-blue-100 text-blue-600 px-2 py-0.5 rounded font-bold">GENERAL</span>}
                                    </div>
                                </div>
                    ...
                    <div className="text-center py-10">
                        ...
                        <p className="mt-4 text-slate-400 font-bold text-sm uppercase">No se encontraron correcciones con ese texto.</p>
                    </div>
                )}
            </div>
        ) : (
